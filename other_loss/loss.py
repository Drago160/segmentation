...
def build_halo_mask(fixed_depth=30, margin=21, min_fragment=10):
    ...
    rr, cc = circle(margin / 2, margin / 2, margin / 2 + 1,
                    margin/2 + 1, shape=(margin, margin))
    structure_element = numpy.zeros((margin, margin))
    structure_element[rr, cc] = 1
    structure_element = numpy.repeat(numpy.expand_dims(
        numpy.expand_dims(structure_element, 0), 0), fixed_depth, 0)
    ...
    def f(label):
        ...
        """
        back = numpy.zeros(
            (label.shape[0], fixed_depth, label.shape[1], label.shape[2]))
        object_list = []
        for i in range(label.shape[0]):
            ...
            if len(pixels) > fixed_depth:
                ...
                warnings.warn(
                    "Not all objects fits in fixed depth", RuntimeWarning)
...
def halo_loss(predicted, labels, weights, obj, k_neg=7.):
    ...
    for i in range(predicted.size(0)):  # Foreach sample
        ...
        global_loss += torch.sum(positive[indices,
                                 torch.arange(0, positive.size(1)).long().to(device)])
